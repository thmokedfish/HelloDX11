...
namespace HelloDX11
{
	...
	protected:
		...
		void CreateResourceWithVertexData(Geometry::MeshData<VertexType, IndexType> mesh);
	public:
		RenderObject();
		void Update();
...
void RenderObject::CreateResourceWithVertexData(Geometry::MeshData<VertexType, IndexType> mesh)
{
	...
	CD3D11_BUFFER_DESC vertexBufferDesc(sizeof(VertexType) * vsize, D3D11_BIND_VERTEX_BUFFER);
	DX::ThrowIfFailed(
	...
	//unsigned short a[10];
	//static IndexType* meshIndices = new IndexType[indexSize];
	...
	indexBufferData.pSysMem = &mesh.indexVec[0];
	indexBufferData.SysMemPitch = 0;
	indexBufferData.SysMemSlicePitch = 0;
	CD3D11_BUFFER_DESC indexBufferDesc(sizeof(IndexType) * m_indexCount, D3D11_BIND_INDEX_BUFFER);
	DX::ThrowIfFailed(
		m_deviceResources->GetD3DDevice()->CreateBuffer(
			&indexBufferDesc,
			&indexBufferData,
